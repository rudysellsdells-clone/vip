...
export default async function PublishingReadyPage({ searchParams }: PageProps) {
  ...
  const publishingImage = publishingImageForAsset(asset);
  ...
  return (
    <WebsitePage>
      ...
      <WebsiteSection
        eyebrow="Visual"
        title={publishingImage.ready ? "Primary image selected for publishing" : "No primary image selected"}
        description="If a primary image is attached to this asset, VIP includes it in the publishing payload so downstream social, blog, and email actions can use it."
      >
        {publishingImage.ready ? (
          <article className={websiteStyles.card}>
            <div style={{ display: "grid", gap: 18, alignItems: "start", gridTemplateColumns: "minmax(0, 220px) minmax(0, 1fr)" }}>
              <img
                src={publishingImage.url ?? ""}
                alt={asset.title ?? "Selected publishing image"}
                style={{ width: "100%", maxWidth: 220, borderRadius: 16, border: "1px solid #dbe5ef", background: "#f8fafc" }}
              />
              <div>
                <div className="flex flex-wrap gap-2">
                  <span className={websiteStyles.badge}>Image attached</span>
                  {publishingImage.imageUse ? (
                    <span className={websiteStyles.badge}>{String(publishingImage.imageUse).replaceAll("_", " ")}</span>
                  ) : null}
                </div>
                <p className={websiteStyles.cardMeta} style={{ marginTop: 12 }}>
                  Visual asset ID: {publishingImage.assetId ?? "Not recorded"}
                </p>
                <p className={websiteStyles.cardMeta}>Prompt asset ID: {publishingImage.promptAssetId ?? "Not recorded"}</p>
                <p className={websiteStyles.cardMeta}>
                  This image URL is what VIP will hand to publishing actions: {publishingImage.url}
                </p>
              </div>
            </div>
          </article>
          ...
          <div className={websiteStyles.empty}>
            No primary image is attached to this asset yet. You can still publish text-only content, or return to the asset and choose a primary visual first.
          </div>
        )}
      </WebsiteSection>
