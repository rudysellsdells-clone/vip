...
import { publishingImageForAsset, type PublishingAsset, type ZapierMcpAssetConfig } from "@/lib/publishing/output-payload";
...
export function buildPublishingPreflightReport({
  ...
  if (["linkedin_post", "facebook_post", "blog_post", "email"].includes(type) && !publishingImage.ready) {
    warnings.push("No primary image is selected for this asset yet. Publishing can continue, but VIP will send text-only output unless you choose a visual first.");
  }
