...
const CURRENT_BEHAVIOR_MARKERS = /\b(currently|today|still|rely|relies|relying|handle|handles|handling|manage|manages|managing|piecemeal|reactive|reactively|between|alongside|sporadic|occasional|ad hoc|workaround|referrals?|posting|outsourc|doing it themselves|owner's time)\b/i;
...
const FALLBACK_TEMPLATE_LANGUAGE = /\b(fragmented execution is limiting progress|reactive, disconnected, or one-off efforts|the workaround may keep activity moving|dependable path for deciding what should happen next|the buyer's decisions and day-to-day actions|desired business outcome|one-off judgment instead of a consistent process|the better approach is not to add more isolated activity|move .{0,120} from recognizing that .{0,160} to viewing)\b/i;
const AWKWARD_STITCHING = /\btrying to\s+(easy|simple|better|effective|consistent|reliable|affordable|branding|marketing|limited|no)\b|\bfor evaluating\s+(demo|audit|consultation|guide|webinar|trial)\b|\.\s*\.|\.,|\bto to\b|\bthe the\b|\band and\b/i;
...
function addIssue(
  ...
  field: StrategyValidationIssue["field"],
  code: string,
  severity: StrategyValidationIssue["severity"],
  message: string,
) {
...
function includesSourceSentence(value: string, source: string) {
  ...
  return (
    (raw.length >= 70 && output.includes(raw)) ||
    similarity(value, source) >= 0.88
  );
}
...
function objectiveUsesNaturalOfferReference(
  value: string,
...
export function validateStrategy({
  ...
  for (const [field, value] of fields) {
    const contract = STRATEGY_FIELD_CONTRACTS[field];
    ...
    if (looksMalformed(value)) {
      addIssue(
        ...
        "malformed_language",
        "critical",
        `${contract.label} contains incomplete, duplicated, stitched, or non-logical language.`,
      );
    }

    if (FALLBACK_TEMPLATE_LANGUAGE.test(value)) {
      addIssue(
        issues,
        field,
        "generic_fallback_template",
        "critical",
        `${contract.label} uses reusable fallback scaffolding instead of campaign-specific strategy.`,
      );
    }

    if (count < contract.minWords || count > contract.maxWords) {
      addIssue(
        issues,
        field,
        "field_length_outside_contract",
        "warning",
        `${contract.label} should be ${contract.minWords}-${contract.maxWords} words; it is ${count}.`,
      );
    }

    if (META_LANGUAGE.test(value)) {
      addIssue(
        issues,
        field,
        "internal_language_leak",
        "critical",
        `${contract.label} exposes internal planning or source language.`,
      );
    }
    ...
    const sourceCandidates = [
      ...
      brief.verifiedOfferFacts.description,
      brief.relatedService.description,
    ...
    if (
      !["offerDeliverables", "proofAndSupport", "primaryCta"].includes(field) &&
      sourceCandidates.some((source) => includesSourceSentence(value, source))
    ) {
      addIssue(
        issues,
        field,
        ...
        "critical",
        `${contract.label} substantially repeats saved source wording instead of producing a strategic conclusion.`,
      );
    }
    ...
    if (forbidden) {
      addIssue(
        issues,
        "offerAuthority",
        `ignored_offer_reintroduced_${field}`,
        "critical",
        `${contract.label} reintroduces the excluded offer term “${forbidden}.” The only promoted offer is “${brief.promotedOffer.name}.”`,
      );
    }
  }
  ...
  if (
    ...
    addIssue(
      ...
      "critical",
      "Target Audience must identify one primary decision-maker and business context, not a market list.",
    );
  }
  ...
  if (
    ...
    VENDOR_MARKERS.test(strategy.buyerSituation)
  ) {
    addIssue(
      issues,
      "buyerSituation",
      ...
      "critical",
      "Buyer Situation must describe the customer trigger, current workaround, breakdown, and urgency—not the vendor or creator situation.",
    );
  }

  if (
    !CAUSAL_MARKERS.test(strategy.coreProblem) ||
    VENDOR_MARKERS.test(strategy.coreProblem)
  ) {
    addIssue(
      issues,
      "coreProblem",
      ...
      "critical",
      "Core Problem must explain the customer-side cause behind the visible symptom.",
    );
  }

  if (
    !CONSEQUENCE_MARKERS.test(strategy.businessConsequence) ||
    VENDOR_MARKERS.test(strategy.businessConsequence)
  ) {
    addIssue(
      issues,
      "businessConsequence",
      "consequence_not_downstream_effect",
      "critical",
      "Business Consequence must explain what the customer loses, risks, delays, or cannot achieve.",
    );
  }

  if (
    GENERIC_LANGUAGE.test(strategy.campaignObjective) ||
    !objectiveUsesNaturalOfferReference(strategy.campaignObjective, brief)
  ) {
    addIssue(
      issues,
      "campaignObjective",
      "objective_not_specific_decision_change",
      "critical",
      "Campaign Objective must state a specific decision change and refer to the resolved offer naturally, without template phrasing.",
    );
  }

  if (
    GENERIC_LANGUAGE.test(strategy.campaignPointOfView) ||
    ...
    addIssue(
      ...
      "critical",
      "Campaign Point of View must contrast the familiar approach with a better one.",
    );
  }
  ...
  if (!MECHANISM_MARKERS.test(strategy.offerExplanation)) {
    addIssue(
      ...
      "critical",
      "Offer Explanation must describe how the resolved promoted offer works and why its mechanism helps.",
    );
  }
  ...
  if (!offerMentioned(strategy.offerExplanation, brief)) {
    addIssue(
      ...
      "critical",
      `Offer Explanation must clearly describe “${brief.promotedOffer.name}.”`,
    );
  }

  if (!brief.verifiedOfferFacts.deliverables.length) {
    const invented = strategy.offerDeliverables.match(
      /\b(report|roadmap|implementation plan|recommendations document|assessment|diagnostic|written analysis|scorecard)\b/i,
    );
    if (invented && brief.promotedOffer.category !== "audit") {
      addIssue(
        issues,
        "offerDeliverables",
        "invented_deliverable",
        "critical",
        `Offer Deliverables invents “${invented[0]},” which is not verified for this offer.`,
      );
    }
  }

  if (
    !brief.approvedProof.length &&
    !/\b(no approved|no quantitative|not supplied|none supplied|not established)\b/i.test(
    ...
    addIssue(
      ...
      "unsupported_proof",
      "critical",
      "Proof and Support must state honestly that no approved proof was supplied instead of implying evidence.",
    );
  }

  if (
    brief.primaryCta &&
    similarity(strategy.primaryCta, brief.primaryCta) < 0.5
  ) {
    addIssue(
      issues,
      "primaryCta",
      "cta_drift",
      "critical",
      `Primary CTA drifted from the approved campaign CTA “${brief.primaryCta}.”`,
    );
  }

  const missingChannel = selectedChannelMissing(
  ...
  if (missingChannel) {
    addIssue(
      ...
      "channel_missing",
      "warning",
      `Channel Direction does not assign a role to the selected channel “${missingChannel}.”`,
    );
  }

  const crossPairs: Array<[StrategyFieldKey, StrategyFieldKey, number]> = [
    ["buyerSituation", "coreProblem", 0.68],
    ["coreProblem", "businessConsequence", 0.68],
    ["campaignPointOfView", "offerExplanation", 0.72],
    ["offerExplanation", "offerDeliverables", 0.7],
  ];

  for (const [left, right, threshold] of crossPairs) {
    if (similarity(strategy[left], strategy[right]) >= threshold) {
      addIssue(
        issues,
        "crossSection",
        `${left}_${right}_overlap`,
        "critical",
        `${STRATEGY_FIELD_CONTRACTS[left].label} and ${STRATEGY_FIELD_CONTRACTS[right].label} repeat the same idea instead of performing separate strategic jobs.`,
      );
    }
  }
...
export function criticalStrategyIssues(issues: StrategyValidationIssue[]) {
  return issues.filter((issue) => issue.severity === "critical");
}

export function formatStrategyValidationIssues(
  issues: StrategyValidationIssue[],
) {
