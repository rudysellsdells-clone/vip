  ...
  try {
    ...
    const { data, error } = await access.writeSupabase
      ...
      .insert({
        ...
        common_pains: lines(body.commonPains),
        desired_outcomes: lines(body.desiredOutcomes),
        objections: lines(body.objections),
        active: true,
        sort_order: 0,
      })
      .select("*")
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unexpected error saving audience." },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unexpected error removing audience." },
      { status: 500 },
    );
  }
}
