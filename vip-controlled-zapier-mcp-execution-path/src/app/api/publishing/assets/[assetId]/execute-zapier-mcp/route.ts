...
async function loadAsset({ assetId }: { assetId: string }) {
  ...
  if (userError || !user) {
    return { supabase, user: null, asset: null, error: "Unauthorized", status: 401 };
  }
  ...
  if (assetError || !asset) {
    return { supabase, user, asset: null, error: "Asset not found.", status: 404 };
  }

  return { supabase, user, asset, error: null, status: 200 };
}
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  if (loaded.error || !loaded.user || !loaded.asset) {
  ...
  if (!isApprovedForPublishing(asset)) {
    return NextResponse.json(
      {
        error: "Only approved, active, latest-version assets can be sent through ZapierMCP.",
        assetState: {
          status: asset.status,
  ...
  try {
    const result = await executeZapierMcpWriteAction({
      ...
      params,
      output: "Return the created record id, url if available, status, and a concise message.",
    });
    ...
    const sentAsset = await markAssetSentToZapier({
      ...
      userId: user.id,
      assetId,
      reference: result.text?.slice(0, 500) ?? JSON.stringify(result.parsedText ?? {}).slice(0, 500),
    });
    ...
    return NextResponse.json({ ok: true, asset: sentAsset, mcp: result });
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to execute ZapierMCP action.",
        payloadPreview: {
          app: config.app,
          action: config.action,
          params,
