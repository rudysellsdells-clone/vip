...
function readableError(value: unknown, fallback = "Unexpected error.") {
  ...
  if (value && typeof value === "object") {
    ...
    return objectValue.error ?? objectValue.message ?? JSON.stringify(value, null, 2);
  }
...
export function SendAssetToZapierMcpButton({ assetId }: { assetId: string }) {
  ...
  async function send() {
    ...
    try {
      ...
      setDetails(payload as Record<string, any>);
  ...
  return (
    <div className={formStyles.form}>
      <div className={formStyles.actions}>
        <button type="button" onClick={send} disabled={sending} className={formStyles.submit}>
